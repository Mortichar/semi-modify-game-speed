const SEMIUtils = (() => {
    ...
    const maxHitOfCurrentEnemy = () => {
        return SEMIUtils.incomingAttackData().incomingDamage;
    };
    ...
    const adjustedMaxHit = () => {
        ...
        const playerIsStunned = SEMIUtils.playerIsStunned();
        const stunnedCase =
            playerIsStunned && !isNaN(SEMIUtils.incomingAttackData().incomingAttack.stunDamageMultiplier);
        if (stunnedCase) {
            maxHit *= SEMIUtils.incomingAttackData().incomingAttack.stunDamageMultiplier;
        }
        ...
        adjustedMaxHit +=
            (combatData.player.isBurning ? Math.floor(SEMIUtils.maxHP() * 0.02) : 0) +
            (combatData.enemy.reflectMelee && attackStyle <= 2
                ? combatData.enemy.reflectMelee * numberMultiplier
                : 0) +
            (combatData.enemy.reflectMagic && attackStyle >= 6
                ? combatData.enemy.reflectMagic * numberMultiplier
                : 0) +
