...
(function () {
    ...
    function script() {
        ...
        let TempContainer = [
            '<div class="font-size-sm font-w600 text-uppercase text-center text-muted"><small id ="',
            '" class="mb-2" style="display:block;clear:both;white-space: pre-line" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title=""></small></div>',
        ];
        let TempContainerAlt = [
            '<div class="font-size-sm text-uppercase text-muted"><small id ="',
            '" class="mt-2" style="display:block;clear:both;white-space: pre-line" data-toggle="tooltip" data-placement="top" data-html="true" title="" data-original-title=""></small></div>',
        ];
        ...
        function secondsToHms(d) {
            ...
            let hDisplayComma =
                h > 0 ? ((m === 0 && s > 0) || (s === 0 && m > 0) ? ' and ' : s > 0 || m > 0 ? ', ' : '') : '';
        ...
        function timeRemaining(skillID) {
            ...
            function calcMasteryXpToAdd(
                ...
                currentTotalSkillXP,
                currentMasteryXP,
                currentPoolMasteryXP,
                currentTotalMasteryLevelForSkill
            ) {
                let xpModifier = 1;
                let xpToAdd =
                    (((calcTotalUnlockedItems(skillID, currentTotalSkillXP) * currentTotalMasteryLevelForSkill) /
                        getTotalMasteryLevelForSkill(skillID) +
                        convertXPToLvl(currentMasteryXP) * (getTotalItemsInSkill(skillID) / 10)) *
                        (timePerAction / 1000)) /
            ...
            function calcPoolXPToAdd(currentTotalSkillXP, masteryXP) {
                if (convertXPToLvl(currentTotalSkillXP) >= 99) {
                    return masteryXP / 2;
                } else {
                    return masteryXP / 4;
                }
            ...
            function calcburnChance(currentMasteryXP) {
                let burnChance = 0;
                if (
                    equippedItems.includes(CONSTANTS.item.Cooking_Skillcape) ||
                    equippedItems.includes(CONSTANTS.item.Max_Skillcape) ||
                    equippedItems.includes(CONSTANTS.item.Cape_of_Completion)
                ) {
                    return burnChance;
            ...
            function adjustTotalChanceToUseRhaelyx(totalChanceToUse, expectedActions, resources) {
                if (chargeUses >= expectedActions || chargeUses >= resources / (totalChanceToUse - RhaelyxChance)) {
                    ...
                    return (
                        resources /
                        (chargeUses / (totalChanceToUse - RhaelyxChance) +
                            (resources - chargeUses) / totalChanceToUse)
                    ); //the denominator is the "real" expectedXP with Rhaelyx, so the "real" chanceToKeep is essentially found through resources/(resources/chanceToKeep)
                }
            }
            ...
            function calcExpectedTime(resources) {
                ...
                while (resources > 0) {
                    ...
                    let totalChanceToUse =
                        1 - masteryChance(currentTotalMasteryXP, chanceToKeep) - currentPreservationAdjustment;
                    ...
                    // Current XP
                    let currentMasteryXP = calcMasteryXpToAdd(
                        currentInterval,
                        currentTotalSkillXP,
                        currentTotalMasteryXP,
                        currentTotalPoolXP,
                        currentTotalMasteryLevelForSkill
                    );
                    ...
                    let resourceActions = Math.round(
                        resources / adjustTotalChanceToUseRhaelyx(totalChanceToUse, resources, resources)
                    );

                    // Mininum actions based on limits
                    let expectedActions = Math.ceil(
                        Math.min(masteryXPActions, skillXPActions, poolXPActions, resourceActions)
                    );
                    ...
                    if (expectedActions == resourceActions) {
                        ...
                        resources -= Math.round(
                            expectedActions *
                                adjustTotalChanceToUseRhaelyx(totalChanceToUse, expectedActions, resources)
                        );
                    }
            ...
            if (timeLeftElement !== null) {
                if (timeLeft !== 0) {
                    ...
                    timeLeftElement.textContent =
                        ...
                        '\r\n Expected finished: ' +
                        DateFormat(finishedTime, timeLeft);
