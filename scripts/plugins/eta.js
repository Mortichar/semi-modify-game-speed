...
(() => {
    ...
    function script() {
             ...
             */
                ...
                getTargetLevel: (skillID, currentLevel) => {
                    return ETASettings.getTarget(
                        currentLevel,
                        ETASettings.GLOBAL_TARGET_LEVEL,
                        ETASettings.TARGET_LEVEL[skillID],
                        99
                    );
                },
                getTargetMastery: (skillID, currentMastery) => {
                    return ETASettings.getTarget(
                        currentMastery,
                        ETASettings.GLOBAL_TARGET_MASTERY,
                        ETASettings.TARGET_MASTERY[skillID],
                        99
                    );
                },
                getTargetPool: (skillID, currentPool) => {
                    return ETASettings.getTarget(
                        currentPool,
                        ETASettings.GLOBAL_TARGET_POOL,
                        ETASettings.TARGET_POOL[skillID],
                        100
                    );
                },
        ...
        function createETA() {
            ...
            ETA.taskComplete = function () {
                ...
                for (let i = 0; i < last.times.length; i++) {
                    ...
                    if (currentTime.current >= lastTime.target) {
                        // current level is higher than previous target
            ...
            function getPercentageInLevel(currentXp, finalXp, type, bar = false) {
                ...
                if (bar === true) {
                    let finalLevelPercentage =
                        finalXp - currentXp > nextLevelXp - currentXp
                ...
                switch (initial.skillID) {
                    ...
                    case CONSTANTS.skill.Thieving:
                        ...
                        if (convertXpToLvl(masteryXp) >= 99) {
                            ...
                            successRate =
                                Math.floor(
                                    (skillLevel[CONSTANTS.skill.Thieving] - npc.level) * 0.7 +
            ...
            function initialVariables(skillID, checkTaskComplete) {
                ...
                if (
                    equippedItems.includes(CONSTANTS.item.Crown_of_Rhaelyx) &&
                    initial.hasMastery &&
                    !initial.isGathering
                ) {
            ...
            function configureHerblore(initial) {
                initial.itemID =
                    herbloreItemData[initial.currentAction].itemID[getHerbloreTier(initial.currentAction)];
            ...
            function configureSummoning(initial) {
                ...
                initial.chanceToDouble = calculateChanceToDouble(
                    CONSTANTS.skill.Summoning,
                    false,
                    0,
                    0,
                    items[initial.itemID]
                );
                return initial;
            }
            ...
            function configureMagic(initial) {
                    ...
                    if (items[equippedItems[CONSTANTS.equipmentSlot.Weapon]].providesRune !== undefined) {
                        if (
                            items[equippedItems[CONSTANTS.equipmentSlot.Weapon]].providesRune.includes(
                                initial.skillReq[i].id
                            )
                        ) {
                            initial.skillReq[i].qty -=
                ...
                if (ALTMAGIC[initial.currentAction].selectItem === 1 && selectedMagicItem[1] !== null) {
                    ...
                    // Spells that doesn't require you to select an item
            ...
            function actionsToBreakpoint(initial, current, noResources = false) {
                ...
                if (initial.hasMastery) {
                    const poolXpToLimit =
                        getLim(initial.poolLim, current.poolXp, initial.targetPoolXp) - current.poolXp;
                ...
                if (initial.skillID === CONSTANTS.skill.Summoning) {
                    ...
                    initial.skillReq
                        .map((_, i) => calcSummoningRecipeQty(initial, current.poolXp, i, masteryLevel))
                        .forEach((x) => {
                ...
                if (!noResources) {
                    ...
                    const actionsWithCharge = Math.min(
                        current.chargeUses,
                        ...Object.getOwnPropertyNames(current.itemQty).map(
                            (id) => current.itemQty[id] / current.skillReqMap[id] / totalChanceToUseWithCharges
                        )
                    );
                    // remaining resources
                    const resWithoutCharge = Math.max(
                        ...
                        Math.min(
                            ...Object.getOwnPropertyNames(current.itemQty).map(
                                (id) =>
                                    current.itemQty[id] / current.skillReqMap[id] -
            ...
            function calcExpectedTime(initial) {
                ...
                const poolXpToPercentage = (poolXp) =>
                    Math.min((poolXp / initial.maxPoolXp) * 100, poolCap).toFixed(2);
            ...
            function setupTimeRemaining(initial) {
                ...
                if (initial.hasMastery) {
                    ...
                    initial.targetPool = ETASettings.getTargetPool(
                        initial.skillID,
            ...
            function getStaticXPBonuses(skill) {
                ...
                xpMultiplier +=
                    (playerModifiers.increasedGlobalSkillXP - playerModifiers.decreasedGlobalSkillXP) / 100;
                return xpMultiplier;
            }
            ...
            function getStaticMXPBonuses(skill) {
                ...
                xpMultiplier +=
                    (playerModifiers.increasedGlobalMasteryXP - playerModifiers.decreasedGlobalMasteryXP) / 100;
                return xpMultiplier;
            }
            ...
            function generateTooltips(initial, ms, results, timeLeftElement, now, flags = {}) {
                ...
                if (!flags.noSkill) {
                    ...
                    tooltip +=
                        ...
                        tooltipSection(
                            ...
                            ms.skill,
                            initial.targetLevel,
                            results.current.targetSkillResources
                        );
                }
                // mastery tooltip
                if (!flags.noMastery && initial.hasMastery) {
                    ...
                    tooltip +=
                        ...
                        tooltipSection(
                            ...
                            results.current.actions[0].targetMasteryResources
                        );
                }
                // pool tooltip
                if (!flags.noPool && initial.hasMastery) {
                    ...
                    tooltip += tooltipSection(
                        ...
                        ms.pool,
                        `${initial.targetPool}%`,
                        results.current.targetPoolResources,
                        prepend
                    );
                }
                // wrap and return
            ...
            const timeLeftToHTML = (initial, target, time, finish, resources) =>
                `Time to ${target}: ${time}<br>ETA: ${finish}` + resourcesLeftToHTML(initial, resources);

            const resourcesLeftToHTML = (initial, resources) => {
                if (ETASettings.HIDE_REQUIRED || initial.isGathering || resources === 0) {
                    return '';
                }
                let req = Object.getOwnPropertyNames(resources)
                    .map((id) => {
                        let src;
                        if (id === '-5') {
                        ...
                        return `<span>${formatNumber(
                            resources[id]
                        )}</span><img class="skill-icon-xs mr-2" src="${src}">`;
                    })
                    .join('');
            ...
            function generateProgressBars(initial, results, idx) {
                ...
                if (initial.hasMastery) {
                    ...
                    // pool
                    const poolProgress =
                        results.finalPoolPercentage > 100
        ...
        function loadETA() {
            ...
            for (let id in SKILLS) {
                if (SKILLS[id].hasMastery) {
                    ...
                    $(bar).after(
                        `<div id="mastery-pool-progress-end-${id}" class="progress-bar bg-warning" role="progressbar" style="width: 0%; background-color: #e5ae679c !important;"></div>`
                    );
                }
            }
            ...
            for (let id in SKILLS) {
                if (SKILLS[id].hasMastery) {
                    ...
                    $(bar).after(
                        `<div id="${id}-mastery-pool-progress-end" class="progress-bar bg-info" role="progressbar" style="width: 0%; background-color: #5cace59c !important;"></div>`
                    );
                }
            }
            ...
            for (let id in SKILLS) {
                if (SKILLS[id].hasMastery) {
                    ...
                    $(bar).after(
                        `<div id="skill-progress-bar-end-${id}" class="progress-bar bg-info" role="progressbar" style="width: 0%; background-color: #5cace59c !important;"></div>`
                    );
                }
            }
            //
    ...
    (function () {
        ...
        function loadScript() {
            if (
                (window.isLoaded && !window.currentlyCatchingUp) ||
