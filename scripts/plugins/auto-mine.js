(() => {
    ...
    const injectAutoMineGUI = () => {
        menuConfig['AM'] = { ignoreCoal: false, order: [] };
        ...
        const barOptions = `
            <div class="custom-control custom-checkbox mb-1">
                <input type="checkbox" class="custom-control-input" id="${id}-ignore-coal" name="${id}-ignore-coal"${menuConfig['AM'].ignoreCoal ? ' checked="checked"' : ''}>
                <label class="custom-control-label" for="${id}-ignore-coal" data-tippy-content="For Superheat IV">Ignore coal</label>
            </div>
            <div>${bars.map((_, i) => btn(i)).join('') + btn(bars.length)}</div>`;
        ...
        const prioritySetting = createBlock(
            ...
            `<div id="SEMI-Auto-Mine-drag-div">${ores.map((_, i) => div(i)).join('')}</div>`);

        const barSelection = createBlock(
            'AutoMine Bar Selection',
            'Mine by ratios of resources in your bank.<br>Ex: 4 coal per 1 mithril. Senses Smithing cape and halves coal requirements if owned.<br>Click current selection to de-select and return to priority-based mining.',
            barOptions);
        ...
        $(`#${id}-ignore-coal`).change(() => {
            menuConfig['AM'].ignoreCoal = event.target.checked;
            storeConfig();
        });
    ...
    const onEnable = () => {
        ...
        injectAutoMineGUI();
        hasSmithingCape = SEMI.hasCapeOn('Smithing') || SEMI.ownsCape('Smithing');
    };
